/*
 * Pinboard.in jQuery plugin
 *
 * Copyright (c) 2013 Liam Clancy
 * Based on the pinboard.com jQuery plugin, Copyright (c) 2007 Paul McLanahan
 *
...
/**
 * Load a list of bookmarks, tags, network members, and/or fans from pinboard.in for
 * a specific user using the pinboard.in JSON feeds (http://pinboard.in/howto/#rss),
 * and without need for any server-side component.
 *
 * @param String user The pinboard.in user who's bookmarks you want to load.
 ...
 * @type jQuery
 * @name Pinboard.in
 * @cat Plugins/Pinboard.in
 *
 */
$.fn.pinboard = function(user,options,tOptions,cbFnc){
	$.defineTag('script');
	var opts = $.extend({'user':user},$.pinboard.opts,options),
		$self = this,
		fn = cbFnc || jQuery.pinboard.parsers[opts.type],
		//url = 'http://feeds.pinboard.com/v2/json/' + (opts.type=='posts'?'':opts.type+'/') + user
		//	+ (opts.type=='posts' && opts.tag? '/'+opts.tag : '') + '?',
		//url = 'http://feeds.pinboard.in/json/u:' + user + '?',
		// request via YQL to avoid Same Origin Policy
		url = 'http://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20json%20where%20url%3D%22http%3A%2F%2Ffeeds.pinboard.in%2Fjson%2Fu%3A' + user + '%3Fcount%3D50%22&format=json&diagnostics=true',
		rOpts = $.extend({raw:'true',callback:name(fn)},$.pinboard.types[opts.type],tOptions);
	...
	function name( callback ) {
		var i = $.pinboard.callbacks.length;

		$.pinboard.callbacks[i] = function( json ) {
			delete $.pinboard.callbacks[i];
			$self.each( function() { callback.apply(this,[json,opts]); } );
		};

		return 'jQuery.pinboard.callbacks['+i+']';
	};
};
...
$.pinboard = {
	...
	parsers : {
      ...
      // shift data structure from YQL response
			...
			$.pinboard.add(this,$[opts.wrapTag]({},lis),opts);
		},
		tags : function(data,opts){
			var lis = [];
			$.each(data,function(tName,tCount){
				var fIcon, oSpan;
				var a_opts = {href:'http://pinboard.com/'+opts.user+'/'+tName};
				if(opts.popopen) a_opts['target'] = '_blank';
				lis[lis.length] = $[opts.itemTag]({},
		...
		network : function(data,opts){
			...
			$.each(data,function(i,name){
				var a_opts = {href:'http://pinboard.com/'+name};
				if(opts.popopen) a_opts['target'] = '_blank';
				lis[lis.length] = $[opts.itemTag]({},
					$.A(a_opts, name)
				);
